...
3. [Data Structures & Protocols](#3-data-structures--protocols)
4. [Algorithms & Processing Logic](#4-algorithms--processing-logic)
...
### 3.1 Chunk Binary Format

**Purpose**: Define how data is chunked and structured.

**Fulfills Requirements**: DR-003

```
Chunk Structure (binary):
┌──────────────────────────────────────────────────────┐
│ Header (20 bytes)                                     │
├──────────────────────────────────────────────────────┤
│ Magic (4 bytes): 0x51524654 ("QRFT")                 │
│ Version (1 byte): 0x01                               │
│ Chunk Index (4 bytes): uint32 big-endian             │
│ Total Chunks (4 bytes): uint32 big-endian            │
│ Data Length (2 bytes): uint16 big-endian             │
│ CRC32 (4 bytes): uint32 big-endian (data only)       │
│ Reserved (1 byte): 0x00                              │
├──────────────────────────────────────────────────────┤
│ Data (variable length, max 2800 bytes)               │
└──────────────────────────────────────────────────────┘

Total max size per chunk: 20 + 2800 = 2820 bytes
```
...
class ChunkHeader:
    ...
    STRUCT_FORMAT = '>4sBIIHIB'  # Big-endian: 4+1+4+4+2+4+1 = 20 bytes
    HEADER_SIZE = 20
    ...
    def unpack(cls, data: bytes) -> 'ChunkHeader':
        ...
        unpacked = struct.unpack(cls.STRUCT_FORMAT, data[:cls.HEADER_SIZE])
        
        if unpacked[0] != b'QRFT':
            raise ValueError("Invalid magic bytes")
            
        return cls(
            magic=unpacked[0],
            version=unpacked[1],
            chunk_index=unpacked[2],
            total_chunks=unpacked[3],
...
@dataclass
class Chunk:
    """Complete chunk with header and data."""
    header: ChunkHeader
    data: bytes
    
    MAX_DATA_SIZE = 2800  # Bytes per chunk
    
    def pack(self) -> bytes:
        """Pack entire chunk to bytes."""
        return self.header.pack() + self.data
        
    @classmethod
    def unpack(cls, data: bytes) -> 'Chunk':
        """Unpack chunk from bytes."""
        header = ChunkHeader.unpack(data)
        chunk_data = data[ChunkHeader.HEADER_SIZE:
                         ChunkHeader.HEADER_SIZE + header.data_length]
        
        # Verify CRC32
        actual_crc = IntegrityUtil.crc32(chunk_data)
        if actual_crc != header.crc32:
            raise IntegrityError(
                f"CRC mismatch: expected {header.crc32}, got {actual_crc}"
            )
            
        return cls(header=header, data=chunk_data)
```

### 3.2 Metadata Frame Format

**Purpose**: Define metadata embedded in first frame.

**Fulfills Requirements**: DR-002, FR-012
...
class Metadata:
    ...
    def from_json(cls, json_str: str) -> 'Metadata':
        ...
        return cls(
            ...
            type=data["type"],
            file=FileMetadata.from_dict(data["file"]),
            transfer=TransferMetadata.from_dict(data["transfer"])
        )
```


### 3.3 Video Format Specification

**Purpose**: Define output video format.

**Fulfills Requirements**: DR-001, NFR-013

**Format**: MP4 container, codec auto-selected (see ADR-003)

**Specifications**:
- **Container**: MP4 (MPEG-4 Part 14)
- **Video Codec**: `avc1` (H.264) where available; fallback to `mp4v` (MPEG-4 Part 2)
- **Color Space**: RGB (no YUV conversion artifacts)
- **Bit Rate**: High quality (minimal compression)
- **Keyframe Interval**: Every frame (no inter-frame compression)
- **Pixel Format**: yuv420p (compatibility)
- **Profile**: Baseline (maximum compatibility)
- **User override**: `--codec [avc1|mp4v]`

**Platform codec availability**:
- macOS: `avc1` (H.264 via VideoToolbox) ✅ native
- Linux: `avc1` requires OpenCV+FFmpeg; `mp4v` is the safe fallback
- Windows: `avc1` available in most OpenCV builds; `mp4v` as fallback

**Rationale**:
- MP4+H.264 is universally supported (VLC, QuickTime, Windows Media Player)
- High bit rate preserves QR code clarity
- All-keyframe encoding ensures every frame is independently decodable
- `mp4v` fallback ensures the tool works on any standard OpenCV pip install
...
✓ **Good Examples**:
```python
# CompressionUtil: Only handles compression
class CompressionUtil:
    @staticmethod
...
# IntegrityUtil: Only handles integrity
class IntegrityUtil:
    @staticmethod
    def sha256(data: bytes) -> str: ...
    @staticmethod
...
class FileProcessor:
    ...
    def generate_qr(self): ...   # QR generation
    def create_video(self): ...  # Video encoding
...
✓ **Good Example**: Error handling extensibility
```python
# Base class defines contract
class QRTransferError(Exception):
    def format_user_message(self) -> str: ...

# New error types extend without modifying base
class IncompleteTransferError(QRTransferError):
    def format_user_message(self) -> str:
        # Custom formatting
        return super().format_user_message() + extra_info
```

✓ **Good Example**: Compression algorithms
```python
# Current: gzip
class CompressionUtil:
    @staticmethod
    def compress(data: bytes, algorithm: str = 'gzip') -> bytes:
        ...
        elif algorithm == 'zstd':  # Future extension
            return zstd.compress(data)
        # Can add new algorithms without changing existing code
```
...
✓ **Good Example**: Video encoders
```python
class VideoEncoderProtocol(Protocol):
    def create(self, frames, output_path, width, height) -> VideoInfo: ...

class OpenCVVideoEncoder:  # Implements protocol
    def create(self, frames, output_path, width, height) -> VideoInfo:
        # OpenCV implementation
        ...

class FFmpegVideoEncoder:  # Also implements protocol
    def create(self, frames, output_path, width, height) -> VideoInfo:
        # FFmpeg implementation
        ...

# Both are substitutable:
encoder: VideoEncoderProtocol = OpenCVVideoEncoder()  # Or FFmpegVideoEncoder()
video = encoder.create(frames, 'out.mp4', 800, 800)
```
...
✓ **Good Example**: Separate protocols
```python
...
# Encoder only needs generator, not detector
class FileEncoder:
    def __init__(self, qr_generator: QRGeneratorProtocol):
        self.qr_gen = qr_generator  # Doesn't need QRDetectorProtocol
```

✗ **Bad Counter-example**:
```python
# DON'T DO THIS - Fat interface
class QRProcessorProtocol(Protocol):
    def generate(self, data: bytes) -> Image: ...
    def detect(self, frame: np.ndarray) -> List[QRData]: ...
    def validate(self, qr_data: bytes) -> bool: ...
    def optimize(self, image: Image) -> Image: ...
    # Clients must implement all methods even if they only need one
```
...
✓ **Good Example**: Encoder depends on abstractions
```python
# High-level module
class FileEncoder:
    def __init__(self, 
                 qr_generator: QRGeneratorProtocol,
                 video_encoder: VideoEncoderProtocol):
        self.qr_gen = qr_generator      # Abstract interface
        self.video_enc = video_encoder  # Abstract interface
        
    # Can swap implementations without changing FileEncoder
```
...
class FileEncoder:
    def __init__(self):
        self.qr_gen = QRGenerator()       # Concrete class
        self.video_enc = VideoEncoder()   # Concrete class
